...
declarative and definable at runtime, per space, by clients. A records
schema expresses required fields, write-once vs author-mutable fields,
author-only delete, apply-time stamps, record-id rules and a
search-extraction annotation without a compiled-in handler; a module
declaration instantiates the module's compiled-in behaviour on a
collection of the type's own.
...
type's static part, may declare it. The check is draft-time only:
compile keeps an applied declaration valid, since the peer that admitted
it was the consumer's own install.

The install root is the module's only carrier. A local write that sets a
user type declaring a reserved module as the type of any row other than
the type's own (`Objects().Create` with `Type`, `SetType`, an `any.type`
op through `Modify`) fails with `handler.ErrValidationReservedCarrier`
(reason `reserved_carrier`), so a client cannot mint a second instance
by retyping an object. `SystemInstall` does not bypass this. A
registered type's static declaration is not a carrier: any object may
take it as its type.

Limits:

- The guard reads this device's catalog snapshot, so a declaration not
  yet compiled locally is not protected.
- Inbound apply is read-tolerant: a peer that lands the type on another
  row is applied, and that row keeps writing (only additions are
  checked).

### Static parts on registered types

A registered type (`config.Config.Types`) declares its parts at boot
(`handler.Type.Parts`) in the shape of a runtime declaration. Each part
...
- Module datasets enter the catalog as if a type object had declared
  ...
  (`Store.ModuleGrants`), and discovery lists them under
  `Owners = [typeId]`.
- Static declarations are validated at `sdk.Open`
  (`ValidateExternalTypes` / `ValidateExternalModules`: slug keys, each
  static dataset named by exactly one part once parts are declared, the
  collection rule, minted names free) and never change at runtime:
  `AddPart` and the other definition writes return `ErrTypeRegistered`.
  `Types().Parts` / `Datasets` return the compiled view with the keys as
  ids.
- A type with `Datasets` and no `Parts` reads back one implicit part per
  dataset, keyed by the dataset name.
- `handler.Type.Hidden` keeps the type out of default listings
  (`TypeInfo.Hidden`).
...
`space.ErrDatasetNotDeclared`. Inbound apply is read-tolerant:
canonical collections register statically on every controller
...
`space`) carry the behavioral declaration. Every zero value is the
...
| Declaration | Meaning |
|---|---|
| `Scope` | write/sync class (synced/derived/local/account); zero = synced |
...
| Declaration | Meaning |
|---|---|
| `Dynamic` | free-form keyspace next to declared fields |
...
apply-time checks read only `ctx.Before`, with zero extra reads); id
...
not required to be declared fields; the annotation is opaque).
...
allocations, asserted by a benchmark (cold restore replays millions of
...
per id range) keeps the contract.

## Definitions on the type object

The `type` meta-type owns a third built-in dataset, `datasets` (next to
`properties` and `shortIds`), registered compiled-in like any other. A
type's parts are CRDT records there:
...
- **Field record** (one per field of a `records` dataset; id derived
  ...
  are pinned; `name`, `description` and the opaque `x-format` descriptor
...
the implicit self grant; they evolve through `Types().AddPart` /
`AddDataset` / `AddDatasetField` / `PatchDataset` / `PatchDatasetField`
with `typeId = rootId`. See `bundles.md § Bundle-declared definitions`.

One record per definition makes concurrent edits merge: creates are
stateless-accepted records, pinning is stateless per path, and distinct
adds converge as distinct records, as in the `properties` dataset.
Cross-record consistency (orphans, author rules without a creator stamp,
the collection rule, module existence) is resolved at catalog compile,
never in hooks: hooks that read sibling
...
- The catalog is a copy-on-write snapshot (`atomic.Pointer`), built
  ...
  applies. Controller construction reads it with one atomic load, with
  no storage scans or declaration compiles (registrations are pre-built
...
- **Inbound**: the apply gate parks a change whose dataset this
  ...
  `_detached` collection (spec §8.2 persist-but-skip).
  ...
  schema may already be fully applied, and nothing else would wake it.
...
writer's schema state parks the data change until it has, so the
detached-changes machinery handles schema and data arriving in either
order.

Changes on a module's canonical collection stamp the module's opaque
`DataVersion`, like compiled-in datasets. The gate lets opaque strings
through: one declaring type's state is the wrong gate for a collection
several types own. Definition writes stamp the hardcoded
`typeDatasetHandler-v1` and are never gated on their own state.

## Generic batch upsert

`Space.Upsert(UpsertBatch)` is the schema-driven ingest endpoint for
any `IdRule: user` dataset:
...
- present → diff **declared-mutable fields only**; identical records
  ...
  (`ErrImmutableFieldChanged`; a silent skip would hide data loss); author-mutable fields pre-checked against the local identity;
...
`description` and the opaque `x-format` bag, verbatim. Compiled-in
datasets carry the same: a `handler.Field` declares `Description` /
`XFormat`, and every SDK-declared dataset (tech space, `bundles`,
`payloads`, the `objects` row's derived root fields) has a description
on each field and a descriptor where one fits (docs/data-structure.md § The `x-format`
descriptor).

`Types().Parts(typeId)` and `Types().Datasets(typeId)` return the
...
- **Deep shape/size validation** is garbage-tolerant per spec §7.2:
...
- A declaration change refreshes the registration (SchemaRev) but never
