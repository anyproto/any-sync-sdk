...
- **Modify display-only fields** (`name`, `description`, `x-key`, `x-kind`, `pos`, `format.ui`, `format.filter`) — allowed, does not mint a shortId. `pos` is the definition's lexid display-order key (property order within the type — the per-definition twin of `format.options.<key>.pos`); reorder is a single-leaf `$set`, concurrent reorders LWW-converge.
