...
- **Per-space boot watermark persists at Close too.** The space-level "highest LastAddSeq seen" row (`spacesync`) is written at the end of a boot catch-up pass AND snapshotted on clean SDK Close for every open, caught-up space — session-live applies already materialized everything below `MaxLastAddSeq`, so without the close snapshot every tree touched during a session sits above the boot value and the next boot force-loads all of them for nothing. Crash ⇒ no snapshot ⇒ boot replay fallback (idempotent, same as the crash-safety bullet above). Gated fail-conservative: skipped for spaces whose boot catch-up never completed this session and for spaces whose treesyncer parked set is non-empty (trees committed to storage but never materialized — the boot replay is their only cross-restart recovery); the write itself never regresses (`persistWatermarkIfAhead`).
