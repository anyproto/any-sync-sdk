...
const (
	...
	probeLen = 4096
	// fileCheckTimeout bounds source selection (the FileCheck sweep) so
	// picking a peer never spikes latency.
	fileCheckTimeout = 1500 * time.Millisecond
...
// non-banned LAN peer reports holding the file in full, else (nil,false).
func (s *Source) SourceFor(ctx context.Context, spaceId string, root cid.Cid) (fetch.CarSource, bool) {
	ids := s.filterBanned(s.peers.LocalPeerIds(spaceId))
	if len(ids) == 0 {
		return nil, false
	}
	sctx, cancel := context.WithTimeout(ctx, fileCheckTimeout)
	defer cancel()
	tried := 0
	for _, id := range ids {
		...
		tried++
		p, err := s.pool.Get(sctx, id)
		if err != nil {
			s.ban(id)
			continue
		}
		if s.holdsFull(sctx, p, spaceId, root) {
			return &peerCar{pool: s.pool, peerId: id, spaceId: spaceId, root: root, ban: s.ban}, true
		}
	}
	return nil, false
}

// holdsFull asks one peer whether it holds the file in full.
func (s *Source) holdsFull(ctx context.Context, p peer.Peer, spaceId string, root cid.Cid) bool {
	var full bool
	err := p.DoDrpc(ctx, func(conn drpc.Conn) error {
		resp, err := filep2p.NewDRPCFileP2PClient(conn).FileCheck(ctx, &filep2p.FileCheckRequest{
			SpaceId:  spaceId,
		...
		for _, f := range resp.Files {
			if bytesEqual(f.RootCid, root.Bytes()) && f.Have == filep2p.Availability_Full {
				full = true
			}
		}
		return nil
...
// like the HTTP source. A read failure bans the peer (so it drops out of
// future selection) and returns an error, which the laddered source
// demotes to HTTP for the rest of the file.
type peerCar struct {
	pool    DialPool
	peerId  string
	spaceId string
	root    cid.Cid
	ban     func(peerId string)
}
...
func (c *peerCar) ReadRange(ctx context.Context, off, length int64) ([]byte, error) {
	...
	if int64(len(data)) != length {
		...
		// the geometry); fall back to HTTP rather than trust it.
		c.ban(c.peerId)
