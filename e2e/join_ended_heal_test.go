...
// a withdrawn join when the account becomes a member another way:
//
//  1. Bob's device A requests to join Alice's space and withdraws
//     (CancelJoin) — A's row is an ended join, StatusDeleted.
//  2. Bob's device B (same account, fresh device key) requests with the
//     same token; Alice accepts; B reaches StatusActive.
//  3. A still reads StatusDeleted — nothing watches the ACL of a space
//     A never loaded. A's Join with the token is refused by the ACL as
//     "already a member", which Join takes as the acceptance signal:
//     the row goes back to joining and the controller's waiter loads
//     the space, so A reaches StatusActive without a new request.
//
...
func TestE2E_JoinEndedHealsOnMembership(t *testing.T) {
	...
	// 3. Device A is blind to the membership until it acts; the token
	// re-join heals it without a new request landing on Alice's side.
	require.Equal(t, space.StatusDeleted, bobStatus(t, ctx, bobA, sp.Id()),
		"an ended join stays deleted on the device that withdrew it")
	_, err = bobA.Spaces().Join(ctx, space.JoinRequest{Invite: token})
	...
	if !waitFor(ctx, 120*time.Second, 1*time.Second, func() bool {
		...
		t.Fatalf("bobA never healed to active after the account became a member")
	}
	got, err := bobA.Spaces().Get(ctx, sp.Id())
	require.NoError(t, err, "bobA: Get after heal")
