...
//
// The derivation is base58(xxhash64(changeId)). changeId is a content-
// addressable CID, so its bytes are uniformly random; xxhash64 preserves that
...
func DeriveRecordId(changeId string) string {
	var buf [8]byte
	binary.BigEndian.PutUint64(buf[:], xxhash.Sum64String(changeId))
