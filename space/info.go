...
type SpaceInfo struct {
	...
	Derived bool
	// Advertise is the per-space p2p advertising switch (on by default):
	// while on, this account's devices publish their global p2p record
	// into the space so the other members can dial them; off hides them
	// from those members. Own devices find each other through the
	// account's discovery record either way. Synced account-wide;
	// written via Space.SetAdvertise.
	Advertise bool
}
