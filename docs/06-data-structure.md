...
- **Built-ins carry the same slice.** The hardcoded properties of `any`, `type` and `spaceIndex` and every SDK-declared dataset field (the tech-space datasets, `payloads`, the `objects` row) declare a `description`, and an `x-format` where the consumer vocabulary names the value — display text, instants, flags. `Types().Properties` and discovery return them exactly like a user definition's; system values (identities, ids, CIDs, opaque objects) carry a description alone.
