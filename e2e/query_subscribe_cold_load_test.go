...
// device B is left unclosed so the test can report instead of wedging.
func TestE2E_QuerySubscribe_ColdObjectLoad(t *testing.T) {
	...
	// load within milliseconds, ahead of headsync's object pull.
	var spB space.Space
	...
	assert.Equal(t, min(10, got.res.Total), len(got.res.Initial), "window must hold the first limit rows of the replayed tree")
	assert.True(t, waitFor(ctx, 30*time.Second, 50*time.Millisecond, func() bool {
	...
	// and reaches the new subscription.
	_, err = spB.Modify(ctx, space.ModifyBatch{
		ObjectId: objId,
		Dataset:  notesDataset,
		Records: []space.RecordModify{{
			Id:  "rec-000",
			Ops: []space.Op{{Type: space.OpSet, Path: "text", Value: "note 000 edited"}},
		}},
	})
	...
	require.NotNil(t, rec, "windowed sub missed the post-subscribe write: %+v", ev)
}
