...
// the background like every other caller) when the key is new or the
// directory holds no profile for the peer. Only the row keyed by the
// other participant counts: the directory is account-wide, so a row
// under any other identity is ignored rather than cached. Silent on a
// spaceIndex tree not present locally yet — the sub fires once it
// arrives.
...
// the profile when the key is new or no profile is cached. A cached key
// says nothing about the profile — an inbox invite for an already
// active row and the synced directory both deliver the key with no
// resolve, and every other resolve kick may have run before the key
// arrived. "No profile" is the resolver's own emptiness test, so a
// name-less profile is not fetched again.
func peerKeyAction(rowKey string, dir techspace.IdentityRecord) (cache, resolve bool) {
	cache = dir.SymKey != rowKey
	unresolved := dir.Name == "" && dir.Description == "" && dir.IconCID == ""
	return cache, cache || unresolved
}
