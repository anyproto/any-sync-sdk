...
func TestSystemPropertiesHandler_AddToSetPullRequireArray(t *testing.T) {
	...
	assert.Equal(t, "ok", rec.GetString(typeAny, propName), "$addToSet on a string field dropped")
	tags := rec.GetArray(typeAny, propTags)
	...
	require.NoError(t, ctrl.ApplyChange(ctx, makeChange(
		"v3", testObjectId, false,
		crdt.Op{Type: crdt.OpPull, Path: []string{typeAny, propName}, Payload: a.NewString("x")},
	)))
	rec = ctrl.Get(ctx, properties.Dataset, testObjectId)
	assert.Equal(t, "ok", rec.GetString(typeAny, propName), "$pull on a string field dropped")
}
...
func TestSystemPropertiesHandler_IncRequiresNumber(t *testing.T) {
	...
	rec := ctrl.Get(ctx, properties.Dataset, testObjectId)
	require.NotNil(t, rec)
	assert.Equal(t, "ok", rec.GetString(typeAny, propName), "$inc on a string field dropped")
	assert.Equal(t, float64(6), rec.GetFloat64(typeAny, propRating), "$inc on the number field applied")
}
...
func TestSystemPropertiesHandler_MultiFieldAllOrNothing(t *testing.T) {
	...
	rec := ctrl.Get(ctx, properties.Dataset, testObjectId)
	if rec != nil {
		assert.Nil(t, rec.Get(typeAny, propName), "good key dropped with the bad one (whole-op drop)")
		assert.Nil(t, rec.Get(typeAny, propRating), "bad key dropped")
	}
	...
	require.NoError(t, ctrl.ApplyChange(ctx, makeChange(
		"v2", testObjectId, true,
		crdt.Op{Type: crdt.OpSet, Payload: good},
	)))
	rec = ctrl.Get(ctx, properties.Dataset, testObjectId)
	require.NotNil(t, rec)
	assert.Equal(t, "hi", rec.GetString(typeAny, propName))
	...
	rec = ctrl.Get(ctx, properties.Dataset, testObjectId)
	require.NotNil(t, rec)
	assert.Equal(t, "hi", rec.GetString(typeAny, propName), "malformed-key op dropped; prior values intact")
}
